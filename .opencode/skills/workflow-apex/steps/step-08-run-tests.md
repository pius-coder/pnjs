...
- Do NOT modify expected behavior just to make a test pass.
- Do NOT delete a valid test.
- Distinguish an environment failure from a code error.
- Do NOT claim an unexecuted test as passing.
- Stop an unproductive loop and explain the blockage.

### 5.6 Track Additional Changes

When a test fix touches a new path not in `{actual_touched_paths}`:

```
1. Resolve the new path's DOX chain
2. Read the relevant surface
3. Add path to {actual_touched_paths}
4. Continue fixing
```
