...
### 4. Maintainability for AI Review

```markdown
## Maintainability Checklist
- [ ] Stable anchors exist (symbols, routes, regions)
- [ ] Surfaces are locatable without full-file reads
- [ ] No artificial physical separation (service/repo layers by convention)
- [ ] No giant file that is impossible to read by zone
- [ ] Dependencies are explicit
- [ ] Non-relevant context is limited
```

### 6. DOX Contract and Context Review

Add this dimension to the review for every modified surface:

```markdown
## DOX Contract Findings

- [ ] Does the change respect DOX Local Contracts?
- [ ] Has a responsibility moved without planned DOX update?
- [ ] Does a new file create a genuine new DOX boundary?
- [ ] Does the plan propose duplicating rules across DOX files?
- [ ] Is any DOX-documented element now incorrect?
- [ ] Are DOX-indicated paths and anchors still valid?
```

DOX findings are flagged for step-09 resolution (do NOT edit DOX here).
...
- CRITICAL: DOX contract violation (safety, auth, data integrity)
- HIGH: Significant bug, will cause issues
- HIGH: DOX pointer drift impacting correctness
