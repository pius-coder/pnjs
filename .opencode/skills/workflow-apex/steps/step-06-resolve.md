...
During resolution:
- A new error directly caused by the fix may be repaired.
- An independent problem must be flagged without opportunistic refactoring.
- Re-run necessary verifications after all corrections.

### 4. Apply Fixes Based on Choice

**Auto-fix Real:**
1. Filter to Real findings only
2. For each: Read the finding's surface context → Apply fix → Verify
3. Track new paths/symbols touched in `{actual_touched_paths}`
4. Log each fix

**Walk through each:**
For each finding in severity order:

1. Read the finding's surface context (symbol + contracts + deps)
2. Do NOT read the full file unless needed
...
**Rejected:** {count}
- F4: False positive (not a real issue)

**Additional paths touched:** {list}
**Additional anchors touched:** {list}
