...
### identity.migrations
- Source: `backend/migrations/sqlite/001-create-identity.sql`, `backend/migrations/mysql/001-create-identity.sql`
- Entry anchors: `001-create-identity` (SQL)
- Contract: Les migrations créent `users`, `sessions` et `audit_logs` avec colonnes, contraintes, FK et index conformes au cahier des charges.
- Storage: `users`, `sessions`, `audit_logs`
- Depends on: `contract.ts` (types partagés UserRole, UserStatus)
- Verification: `bun run migrate:up && bun run migrate:down` (quand le runner existe)
